...
import { BotIcon, SendIcon, UserIcon, ShieldCheckIcon, MessageCircleIcon } from './icons';
...
const WhatsAppBotScreen: React.FC = () => {
    ...
    const handleSimulateMessage = async () => {
        ...
        if (simulationResult) {
            ...
            const botReplyText = await gemini.getWhatsAppBotReply(newMessage.text, client, currentChat?.messages || []);
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 flex-shrink-0">
                ...
                <button
                    ...
                    disabled={isLoading}
                    className="bg-orange-600 text-white font-semibold px-4 py-2 rounded-lg hover:bg-orange-700 disabled:bg-slate-400 transition-colors flex items-center justify-center min-w-[220px]"
                >
                    {isLoading ? (
                        <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                    ) : (
                       <> <SendIcon /> <span className="ml-2">Simular Nova Mensagem</span></>
                    )}
                </button>
            </header>
